...
int main()
{
	...
	try
	{
		...
		if ((hPipe = CreateNamedPipe(L"\\\\.\\pipe\\Tube",
			PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_WAIT, 1, 
			NULL, NULL, INFINITE, NULL)) == INVALID_HANDLE_VALUE)
		...
		while (true)
		{
			...
			if (!ReadFile(hPipe, rbuf, sizeof(rbuf), &rb, NULL))
				throw "Eroor: ReadFile";

			cout << rbuf << endl;

			if (!WriteFile(hPipe, wbuf, sizeof(wbuf), &wb, NULL))
